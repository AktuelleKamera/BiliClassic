...
public class DmImgParamUtil {
    ...
    /**
     * 给 URL 添加 WBI 签名参数（手动拼接，不依赖 okhttp）
     */
    public static String getDmImgParamsUrl(String origUrl) {
    ...
    private static String generateDmImgInter() throws JSONException {
        ...
        result.put("ds", ds);
        result.put("wh", new JSONArray().put(f114(width, height)));
        result.put("of", new JSONArray().put(f514(y, x)));
